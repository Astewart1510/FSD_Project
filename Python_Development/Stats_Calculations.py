...
betas_t = np.array(np.transpose(betas))
...
pfBeta = np.dot(weights_t, betas) #Portfolio_Beta

# np.dot(betas, np.dot(betas_t, mktVol**2)) = sum(betas*betas_t*(mktVol**2)) = 1.1358
sysCov = betas*betas_t*(mktVol**2) #Systematic_Covariance_Matrix

pfSysVol = np.dot(weights_t, np.dot(betas, np.dot(betas_t, weights)))*(mktVol**2) #Portfolio_Systematic_Variance

specCov = S**2 #Specific_Covariance_Matrix

pfSpecVol = np.dot(np.dot(weights_t,S**2),weights) #Portfolio_Specific_Variance

totCov = sysCov + specCov #Total_Covariance_Matrix

pfVol = pfSysVol + pfSpecVol # Portfolio_Variance

CorrMat = D_inverse*(sysCov + specCov)*D_inverse #Correlation_Matrix
